...
export function addBookToList(listId: string, data: {
  ...
  return http.post<{
    ...
    message: string
    data: BookListItem
  }>(`/api/v1/booklists/${listId}/books`, data)
}
...
export function getMyBookListStats() {
  return http.get<{
    code: number
    message: string
    data: MyBookListStats
  }>('/api/v1/booklists/my/stats')
}
